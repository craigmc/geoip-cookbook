...
last_updated = node['geoip']['city_db_updated'] ? node['geoip']['city_db_updated'] : 0
now = Time.new.to_i

if (Time.new.to_i - last_updated) > 60 * 60 * 30 then
	node.set['geoip']['city_db_updated'] = Time.new.to_i
	bash "remove old database" do
		user "root"
...
remote_file "#{node.geoip.directory}#{node.geoip.city_db_file}.download.gz" do
  source   node.geoip.city_db_download_link
  checksum node.geoip.city_checksum
  mode     0644
